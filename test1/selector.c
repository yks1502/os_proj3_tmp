...
int main(int argc, char **argv)
{
	...
	struct rotation_range write_lock;
	write_lock.degree = 360;
	write_lock.range=10;
	...
	while (1) {
		...
		orient_write_lock(write_lock.degree, write_lock.range);
		printf(" Acquired !\n");
		...
		if (buf == NULL) {
			...
			fclose(integer_file);
			orient_write_unlock(write_lock.degree, write_lock.range);
			continue;
		}

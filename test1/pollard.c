...
static int read_integer(mpz_ptr result)
{
	...
	struct rotation_range read_lock;
	read_lock.degree = 360;
	read_lock.range=10;

	printf("Attempting to take read lock...");
	orient_read_lock(read_lock.degree, read_lock.range);
	printf("Acquired !\n");
	...
	orient_read_unlock(read_lock.degree, read_lock.range);
	printf("Released !\n");
	return ret_code;
}
